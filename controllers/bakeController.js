...
exports.startBake = async (req, res) => {
  ...
  try {
    ...
    const recipeCheckQuery = `
      ...
      FROM Recipe r
      LEFT JOIN RecipeStep rs ON r.recipe_id = rs.recipe_id AND rs.step_order = (SELECT MIN(inner_rs.step_order) FROM RecipeStep inner_rs WHERE inner_rs.recipe_id = r.recipe_id)
    ...
    if (recipeCheckResult.rows.length === 0) {
      await client.query("ROLLBACK"); return res.status(404).json({ message: "Recipe not found or not accessible." });
    }
    const recipeDetails = recipeCheckResult.rows[0];
    if (!recipeDetails.first_recipe_step_id) {
      await client.query("ROLLBACK"); return res.status(400).json({ message: "Recipe has no steps." });
    }
    ...
    const { bake_log_id: newBakeLogId, bake_start_timestamp: bakeStartTime } = bakeLogResult.rows[0];
    ...
    const bakeStepLogResult = await client.query(bakeStepLogQuery, [newBakeLogId, recipeDetails.first_recipe_step_id, recipeDetails.first_step_order, recipeDetails.first_step_name, recipeDetails.first_step_duration]);
    const { bake_step_log_id: newBakeStepLogId, actual_start_timestamp: firstStepStartTime } = bakeStepLogResult.rows[0];

    await client.query("COMMIT");
    console.log(`   Bake Log ID ${newBakeLogId} started for recipe "${recipeDetails.recipe_name}".`);
    res.status(201).json({
      message: "Bake session started.", bakeLogId: newBakeLogId, currentBakeStepLogId: newBakeStepLogId,
      firstStepDetails: {
        recipe_step_id: recipeDetails.first_recipe_step_id, step_id: recipeDetails.first_step_id,
        step_name: recipeDetails.first_step_name, step_order: recipeDetails.first_step_order,
        planned_duration_minutes: recipeDetails.first_step_duration, notes: recipeDetails.first_step_notes,
        target_temperature_celsius: recipeDetails.first_step_temp, actual_start_timestamp: firstStepStartTime,
        stretch_fold_interval_minutes: recipeDetails.first_stretch_fold_interval,
      },
      bakeStartTimestamp: bakeStartTime, recipeName: recipeDetails.recipe_name,
    });
  } catch (error) {
...
// POST /api/bakes/:bakeLogId/steps/complete
  ...
  try {
    ...
    const nextStepQuery = `
      SELECT rs.recipe_step_id, rs.step_id, s.step_name, rs.step_order, rs.duration_override AS planned_duration_minutes, 
             rs.notes, rs.target_temperature_celsius, rs.stretch_fold_interval_minutes
    ...
    if (nextStepResult.rows.length > 0) {
      const nextStepDetails = nextStepResult.rows[0];
      ...
      const nextLogResult = await client.query(insertNextLogQuery, [bakeLogId, nextStepDetails.recipe_step_id, nextStepDetails.step_order, nextStepDetails.step_name, nextStepDetails.planned_duration_minutes]);
      const { bake_step_log_id: nextBakeStepLogId, actual_start_timestamp: nextStepStartTime } = nextLogResult.rows[0];
      await client.query("COMMIT");
      console.log(`   Next Step Log ID ${nextBakeStepLogId} initiated.`);
      res.status(200).json({ message: "Step completed, next initiated.", nextBakeStepLogId, nextStepDetails: { ...nextStepDetails, actual_start_timestamp: nextStepStartTime }});
    } else {
...
// PUT /api/bakes/:bakeLogId/status
exports.updateBakeStatus = async (req, res) => {
  ...
  try {
    ...
    if (status === 'abandoned' || status === 'completed') setClauses.push("bake_end_timestamp = NOW()");
    else if (status === 'active' || status === 'paused') setClauses.push("bake_end_timestamp = NULL");
    ...
    if (result.rowCount === 0) {
      await client.query("ROLLBACK"); return res.status(404).json({ message: "Bake session not found or no change." });
    }
    await client.query("COMMIT");
...
exports.getActiveBakes = async (req, res) => {
  ...
  try {
    const query = `
      SELECT ubl.bake_log_id, ubl.recipe_id, r.recipe_name, ubl.bake_start_timestamp, ubl.status,
             (SELECT json_build_object(
                  'bakeStepLogId', ubsl.bake_step_log_id, 'recipeStepId', ubsl.recipe_step_id,
                  'stepOrder', ubsl.step_order, 'stepName', ubsl.step_name,
                  'plannedDurationMinutes', ubsl.planned_duration_minutes,
                  'actualStartTimestamp', ubsl.actual_start_timestamp,
                  'notes', rs.notes, 'description', s.description,
                  'targetTemperatureCelsius', rs.target_temperature_celsius,
                  'stretchFoldIntervalMinutes', rs.stretch_fold_interval_minutes
      ...
      FROM UserBakeLog ubl JOIN Recipe r ON ubl.recipe_id = r.recipe_id
    ...
    const activeBakes = rows.map(bake => ({ ...bake, currentStep: bake.currentStepDetails }));
    console.log(`   Found ${activeBakes.length} active bake(s) for User ID ${dbUserId}.`);
