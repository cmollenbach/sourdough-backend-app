...
(NULL, 'My First Loaf Tin Sourdough', 'A simple sourdough recipe perfect for beginners, baked in a loaf tin. Easy to handle, minimal shaping.', 900, 1, 75, 2.0, TRUE); -- Hydration: 75, Salt: 2.0
...
((SELECT recipe_id FROM "Recipe" WHERE recipe_name = 'My First Loaf Tin Sourdough' AND user_id IS NULL), 1, 1, 20, 100, 'Build your levain. It should be active and bubbly.', 360, 24, NULL), -- Levain hydration 100%
...
WHERE r.recipe_name = 'My First Loaf Tin Sourdough' AND r.user_id IS NULL AND rs.step_order = 2;
INSERT INTO "StageIngredient" (recipe_step_id, ingredient_id, bakers_percentage, is_wet)
SELECT rs.recipe_step_id, 6, 0.02, FALSE -- Salt as StageIngredient still 0.02 (decimal for BP calculation)
FROM "RecipeStep" rs JOIN "Recipe" r ON rs.recipe_id = r.recipe_id
WHERE r.recipe_name = 'My First Loaf Tin Sourdough' AND r.user_id IS NULL AND rs.step_order = 2;
...
INSERT INTO "Recipe" (user_id, recipe_name, description, target_weight, target_weight_unit_id, target_hydration, target_salt_pct, is_base_recipe) VALUES
(NULL, 'Simple Sourdough Focaccia', 'A high-hydration, very forgiving focaccia. Great for using up starter and learning to handle wet dough.', 1000, 1, 80, 2.5, TRUE); -- Hydration: 80, Salt: 2.5

INSERT INTO "RecipeStep" (recipe_id, step_id, step_order, contribution_pct, target_hydration, notes, duration_override, target_temperature_celsius, stretch_fold_interval_minutes) VALUES
((SELECT recipe_id FROM "Recipe" WHERE recipe_name = 'Simple Sourdough Focaccia' AND user_id IS NULL), 1, 1, 15, 100, 'Build your levain.', 360, 24, NULL), -- Levain hydration 100%
...
WHERE r.recipe_name = 'Simple Sourdough Focaccia' AND r.user_id IS NULL AND rs.step_order = 2;
INSERT INTO "StageIngredient" (recipe_step_id, ingredient_id, bakers_percentage, is_wet)
SELECT rs.recipe_step_id, 6, 0.025, FALSE -- Salt as StageIngredient still 0.025 (decimal for BP calculation)
FROM "RecipeStep" rs JOIN "Recipe" r ON rs.recipe_id = r.recipe_id
WHERE r.recipe_name = 'Simple Sourdough Focaccia' AND r.user_id IS NULL AND rs.step_order = 2;
...
INSERT INTO "Recipe" (user_id, recipe_name, description, target_weight, target_weight_unit_id, target_hydration, target_salt_pct, is_base_recipe) VALUES
(NULL, 'Alexandra''s Simple Artisan Loaf (Adapted)', 'A beginner-friendly approach to a classic artisan loaf with a good crust and open crumb.', 950, 1, 72, 2.0, TRUE); -- Hydration: 72, Salt: 2.0

INSERT INTO "RecipeStep" (recipe_id, step_id, step_order, contribution_pct, target_hydration, notes, duration_override, target_temperature_celsius, stretch_fold_interval_minutes) VALUES
((SELECT recipe_id FROM "Recipe" WHERE recipe_name = 'Alexandra''s Simple Artisan Loaf (Adapted)' AND user_id IS NULL), 1, 1, 20, 100, 'Prepare your levain until active and bubbly.', 300, 25, NULL), -- Levain hydration 100%
...
SELECT 'Base recipes and their steps populated successfully with percentage values as whole numbers!' AS status;